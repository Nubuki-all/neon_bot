...
class TikmateAsync(AsyncClient):
    ...
    async def get_media(self, url: str) -> str:
        async with aiohttp.ClientSession() as session:
            ...
            token_match = dict(re.findall('name="(token)" value="(.*?)"', token_res.text))
    ...
    async def download_tikmate(self, url: str,
        output_dir: str = ".",
        quiet: bool = False,
        progress_callback: Callable[[int, int, str], Awaitable[None]] | None = None,
    ) -> list[DownloadResult]:
...
def parse_tiktok_media(decoded_html_page: str) -> list[dict]:
    ...
    html_match = re.search(r'innerHTML\s*=\s*"(.*?)";\s*parent\.document', decoded_html_page, re.DOTALL)
    if html_match:
    ...
    caption_elem = soup.select_one('div.videotikmate-middle p, .video-info p, p[itemprop="description"]')
    ...
    if video_download_btn:
        thumbnail_tag = soup.select_one('.thumbnail img, .image-tikmate img, img[alt="avatar"]')
