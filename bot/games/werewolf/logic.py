...
def win_condition(game):
    ...
    if all_lovers:
        ...
        len([p for p in players_alive if p.role == "serial killer"])
        >= len(players_alive) / 2
    ):
        ...
        if game.mode == "evilvillage":
            if not teams["village"]:
                if any(p.role == "monster" for p in players_alive):
                    ...
                    len(players_alive)
                    - len([p for p in players_alive if p.role in ["cultist", "minion"]])
                ):
                    win_team = "no win"
        ...
        len(
            [
                p
                for p in players_alive
                if p.team == "wolf" or p.role == "serial killer" or p.role == "monster"
            ]
        )
        == 0
    ):
        # Simplification: if monster is dead, villagers win if wolves/SK are
        # dead
        if game.mode == "evilvillage":
            win_team = "village"
    ...
    for uid in set(killed):
        ...
        if p and p.is_alive:
            ...
            if p.role == "vengeful ghost":
                p.vengeance_against = "wolf"  # simplified
