...
def win_condition(game):
    ...
    def get_plurals(item, sgl="", plr="s", invert=False):
        if invert: return "" if item > 1 else "s"
        return plr if item > 1 else sgl

    living_monster_num = len([p for p in players_alive if p.role == "monster"])
    living_cultists_minions = [p for p in players_alive if p.role in ["cultist", "minion"]]
    living_sk_num = len([p for p in players_alive if p.role == "serial killer"])

    for player in players_alive:
        if player.is_injured:
            if player.team == "wolf" and player.role not in ["cultist", "minion"] and "entranced" not in player.other:
                continue
    ...
    if all_lovers:
        win_team, win_lore = "lovers", "Game over! The remaining villagers through their inseparable love for each other have agreed to stop all of this senseless violence and coexist in peace forever more. All remaining players win."
    elif len([p for p in players_alive if p.role == "succubus" or "entranced" in p.other]) == len(players_alive):
        win_team = "succubi"
        win_lore = "Game over! The succub{} completely enthralled the village, making them officers in an ever-growing army set on spreading their control and influence throughout the entire world.".format("i" if len([p for p in game.players.values() if p.original_role == "succubus"]) > 1 else "us")
        ...
        win_lore = "Game over! Everyone has fallen victim to the charms of the piper{0}. The piper{0} lead{1} the villagers away from the village, never to return...".format(get_plurals(p_num), get_plurals(p_num, invert=True))
    elif living_sk_num >= len(players_alive) / 2:
        if living_monster_num > 0:
            win_team, win_lore = "monster", "Game over! The serial killer{0} stabbed all those in the village, except the monster{1}, causing the monster{1} to win.".format(get_plurals(living_sk_num), get_plurals(living_monster_num))
        else:
            win_team = "serial killers"
            win_lore = "Game over! The serial killer{0} stabbed all those in the village! The serial killer{0} walk{1} off, in the hope to successfully do the same at another location.".format(get_plurals(living_sk_num), get_plurals(living_sk_num, invert=True))
        ...
        if game.mode == "evilvillage":
            if not teams["village"]:
                if living_monster_num > 0:
                    win_team = "monster"
                    win_lore = "Game over! All the villagers are dead! As the cultists rejoice, they get destroyed by the monster{0}, causing the monster{0} to win.".format(get_plurals(living_monster_num))
            ...
            win_team = "monster"
            win_lore = "Game over! The number of uninjured villagers is equal or less than the number of living wolves! The wolves overpower the villagers but then get destroyed by the monster{0}, causing the monster{0} to win.".format(get_plurals(living_monster_num))
        else:
            win_team = "wolf"
    ...
    elif len([p for p in players_alive if p.team == "wolf" and p.role not in ["cultist", "minion"]]) == 0 and living_sk_num == 0:
        if living_monster_num > 0:
            win_team = "monster"
            win_lore = "Game over! All the wolves are dead! As the villagers start preparing the BBQ, the monster{0} quickly kill{1} the remaining villagers, causing the monster{0} to win.".format(get_plurals(living_monster_num), get_plurals(living_monster_num, invert=True))
        else:
            if len(living_cultists_minions) == teams["wolf"] and game.mode == "evilvillage":
                win_team = "village"
        ...
        if living_monster_num > 0:
            win_team = "monster"
            win_lore = "Game over! The number of uninjured cultists is equal or less than the number of living villagers! as the villagers regain control over the village, the monster{0} quickly kill{1} the remaining villagers, causing the monster{0} to win.".format(get_plurals(living_monster_num), get_plurals(living_monster_num, invert=True))
            ...
            if not is_night_zero and player.is_alive:
                if player.role in ["seer", "oracle", "augur"]:
                    ...
                    if player.role == "seer": await bot.client.send_message(player.user_id, f"Your vision reveals that {target.name} is a **{target.display_role}**.")
                        ...
                        await bot.client.send_message(player.user_id, f"{target.name} has a {aura} aura.")
...
async def process_night(game):
    ...
    for uid in set(killed):
        ...
        if p and p.is_alive:
            ...
            if p.role == "vengeful ghost": p.vengeance_against = "wolf"
