...
def win_condition(game):
    ...
    for player in players_alive:
        if player.is_injured:
            if player.role in ACTUAL_WOLVES and player.role not in ["cultist", "minion"] and "entranced" not in player.other:
                continue
    ...
    if pipers_alive and (len(pipers_alive) + len(charmed_alive)) == len(players_alive):
         return "pipers", "Game over! Everyone has fallen victim to the charms of the piper.", [p.user_id for p in players_alive]

    # Serial Killer win
    sk_alive = [p for p in players_alive if p.role == "serial killer"]
    if sk_alive and len(sk_alive) >= len(players_alive) / 2:
         return "serial killers", "Game over! The serial killer stabbed all those in the village!", [p.user_id for p in sk_alive]

    monster_alive = [p for p in players_alive if p.role == "monster"]

    if teams["village"] + teams["neutral"] <= teams["wolf"] and not (game.mode == "evilvillage" and teams["village"]):
        if monster_alive:
            win_team = "monster"
...
async def night_phase(game, is_night_zero=False):
    ...
    for p in game.players_alive_list:
        if p.role == "shaman":
            ...
            await bot.client.send_message(p.user_id, f"You have a random totem tonight.")
    tasks = []
    for p in game.players_alive_list:
        should_act = False
        if is_night_zero:
            if p.role in ["matchmaker", "clone"]: should_act = True
        else:
            if any(p.role in COMMANDS_FOR_ROLE.get(cmd, []) for cmd in ["kill", "see", "give", "guard", "observe", "visit", "hex", "curse", "charm"]):
                should_act = True
        if should_act: tasks.append(request_night_action(game, p, is_night_zero))
...
async def request_night_action(game, player, is_night_zero):
    ...
    if player.role == "matchmaker":
        for p in game.players_alive_list: options[p.user_id] = [f"{p.name} (#{p.id})"]
    ...
    if player.role == "matchmaker": prompt = "Choose TWO players to be lovers (Select 2):"
    elif player.role == "piper": prompt = "Choose up to TWO players to charm (Select 2):"
    try:
        ...
        if response:
            ...
            if "pass" in response:
                 ...
                 return
            target_names = ", ".join([game.get_player_by_user_id(uid).name if uid != "pass" else "Pass" for uid in response])
          ...
          if p.role == "matchmaker" and p.target:
               ...
               if len(uids) == 2:
                    ...
                    if p1 and p2:
                         ...
                         await bot.client.send_message(p1.user_id, f"You are now in love with {p2.name}!"); await bot.client.send_message(p2.user_id, f"You are now in love with {p1.name}!")
...
async def process_night(game):
    ...
    for p in game.players_alive_list:
         if p.role == "hag" and p.target:
              ...
              if target: target.add_other("hexed")
    for p in game.players_alive_list:
    ...
    for p in game.players_alive_list:
         if p.role == "succubus" and p.target and p.target != "pass":
              ...
              if "bishop" not in target.templates:
                   target.add_other("entranced"); await bot.client.send_message(target.user_id, "You have been entranced by a succubus!")
    for p in game.players_alive_list:
         if p.role == "piper" and p.target and p.target != "pass":
              for t_id in p.target.split(","):
                   target = game.get_player_by_user_id(t_id)
                   if target: target.add_other("charmed")
    killed_msg = ""
    for uid in set(killed):
        p = game.get_player_by_user_id(uid)
        if p and p.is_alive:
            if p.role == "lycan" or "lycanthropy_totem" in p.other:
                 p.role = "wolf"; p.team = "wolf"; await bot.client.send_message(p.user_id, "You were attacked but turned into a *wolf* instead of dying!"); continue
            ...
            if p.role == "mad scientist":
                 ...
                 for offset in [-1, 1]:
                      ...
                      if neighbor and neighbor.is_alive:
                           neighbor.is_alive = False; killed_msg += f"The mad scientist's chemicals killed *{neighbor.name}*!\n"
            if p.role == "time lord":
                 game.day_timeout //= 2; game.night_timeout //= 2; await game.send_lobby("The Time Lord has fallen! Time begins to speed up...")
            for cloner in game.players_alive_list:
                 if cloner.role == "clone" and cloner.cloning_target == p.user_id:
                      cloner.role = p.role; cloner.team = p.team; await bot.client.send_message(cloner.user_id, f"Your clone target died. You are now a *{cloner.role}*!")
            if p.loves:
                for l_id in p.loves:
                    lover = game.get_player_by_user_id(l_id)
                    if lover and lover.is_alive:
                        lover.is_alive = False; killed_msg += f"*{lover.name}* committed suicide out of grief for their lover.\n"
...
async def day_phase(game):
    ...
    await game.send_lobby(f"It is now *Day {game.night_num}*. Discussion is open. Use `.lynch <id>` to vote.")
        ...
        if lynched_uid != "abstain":
            ...
            if p:
                if "mayor" in p.templates and "revealed_mayor" not in p.other:
                    ...
                    p.is_alive, p.is_lynched = False, True; p.add_other("lynched"); await game.send_lobby(f"The village has lynched *{p.name}*. They were a *{p.role}*.")
                    if p.role == "fool": await end_game(game, ("fool", "The fool has won by being lynched!", [p.user_id])); return
        else: await game.send_lobby("The village has decided to abstain today.")
    else: await game.send_lobby("No one was lynched today.")
